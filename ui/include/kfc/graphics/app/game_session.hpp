...
public:
    ...
    /// is prompted for (without echoing) rather than passed on the command
    /// line, to keep it out of shell history. Without --server, starts local
    /// single-player. Board dimensions are known immediately in both modes,
    /// but networked play does not connect until connect() is called. Throws
    /// std::runtime_error if config or board file can't be loaded.
